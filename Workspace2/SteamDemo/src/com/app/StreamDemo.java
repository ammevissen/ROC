...
public class StreamDemo {
	...
	public static void main(String[] args) {
		...
		List<String> length5andAboveFruits=fruits.stream().filter(x->x.length()>=5).map(x->x.toUpperCase()).sorted(Collections.reverseOrder()).collect(Collectors.toList());
		System.out.println(length5andAboveFruits);
